...
def welderx():
    ...
    def buildSerial():
        try:
            ...
            print (welderSerial)
            f = open("/tmp/rw_arm_select", "w+")
            f.write(str(arm_select))
            f = open("/tmp/rw_theta_target", "w+")
            f.write(str(theta_target))
            f = open("/tmp/rw_r_target", "w+")
            f.write(str(r_target))
            f = open("/tmp/rw_left_z", "w+")
            f.write(str(left_z))
            f = open("/tmp/rw_right_z", "w+")
            f.write(str(arm_select))
            f = open("/tmp/rw_distance", "w+")
            f.write(str(distance))
            f = open("/tmp/rw_speed", "w+")
            f.write(str(speed))
            f = open("/tmp/rw_trim", "w+")
            f.write(str(trim))
            f = open("/tmp/rw_wire_speed", "w+")
            f.write(str(wire_speed))
            f = open("/tmp/rw_argon", "w+")
            f.write(str(argon))
            f = open("/tmp/rw_outriggers", "w+")
